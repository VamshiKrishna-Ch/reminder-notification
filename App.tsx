...
const App = () => {
  ...
  function scheduleNotifications(deadlineTimestamp, frequency, taskId, title) {
    ...
    // Calculate the intervals for notifications
    for (let i = 1; i <= frequencyInt; i++) {
      ...
      if (i === frequencyInt) {
        notificationTime.setTime(deadlineDate.getTime() - 0.5 * 60 * 1000);
      }

      if (notificationTime > now) {
        PushNotification.localNotificationSchedule({
          channelId: "12314",
          title: `Reminder ${i} - Task Deadline Approaching`,
  ...
  return (
    <NavigationContainer>
      <Stack.Navigator initialRouteName="Login">
        ...
        <Stack.Screen name="TaskPage" component={TaskPage} options={{ title: 'Manage Tasks' }} />
        <Stack.Screen name="HomePage" component={HomePage} />
        <Stack.Screen name="TaskListPage" component={TaskListPage} options={{ title: 'My Tasks' }} />
      </Stack.Navigator>
    </NavigationContainer>
  );
};
